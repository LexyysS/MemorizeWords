...
const Card = () => {
  const { selectedWord, deleteWord, setWarning, setShowMoreDetails } =
    useStore();
    ...
    <div className=" flex flex-col justify-between p-4 gap-4 w-full h-full ">
      ...
      <div className="flex justify-center items-center w-full gap-2 ">
        <button
          ...
          className="bg-red-500 hover:bg-red-600 p-3 flex justify-center items-center rounded-full"
        >
          <TrashIcon className="w-6 h-6 text-white" />
        </button>
        <button
          onClick={() => setShowMoreDetails()}
          className="bg-blue-600 w-full hover:bg-blue-700 text-white font-bold p-3 rounded-full"
        >
          Saber más{" "}
        </button>
      </div>
